...
#define ensure_writable(sz)                                  \
    if ((ws->offset < 0) || (ws->offset + (sz) >= ws->size)) \
    {                                                        \
        ws->offset = -1;                                     \
        return;                                              \
    }
...
#define write_byte_and_data(b, data, sz)        \
    ws->buf[ws->offset++] = (b);                \
    rt_memcpy(ws->buf + ws->offset, (data), (sz)); \
    ws->offset += sz;
...
static inline unsigned char is_indefinite(const cn_cbor *cb)
{
...
static void _write_positive(cn_write_state *ws, cn_cbor_type typ, uint64_t val)
{
    ...
    CBOR_ASSERT((size_t)typ < sizeof(_xlate));
...
void _encoder_visitor(const cn_cbor *cb, int depth, void *context)
{
    ...
    case CN_CBOR_BYTES:
        ...
        rt_memcpy(ws->buf + ws->offset, cb->v.str, cb->length);
        ws->offset += cb->length;
        break;
