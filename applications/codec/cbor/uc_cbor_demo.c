...
codec_data_list_t *codec_demo_decode(unsigned char *encoded_in, unsigned short len, unsigned short *list_num_out)
{
    ...
    rt_memset(decode_data, 0, sizeof(codec_data_list_t) * DEMO_LIST_MAX_NUM);
    ...
    for (unsigned short i = 0; i < DEMO_LIST_MAX_NUM; i++)
    {
        ...
        if (val)
        {
            ...
            rt_memcpy(decode_data->data_ptr, val->v.str, val->length);
...
void codec_demon_test(void)
{
    ...
    rt_memset(encode_data, 0, sizeof(codec_data_list_t) * DEMO_LIST_MAX_NUM);
