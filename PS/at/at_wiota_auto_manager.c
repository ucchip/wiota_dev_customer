...
static void at_wiota_print_freq_list(void)
{
    ...
    {
        rt_kprintf("addr 0x%x freq %d snr %d rssi %d is_sync %d\n",
...
{
    t_freq_list_manager *temp_node;
    rt_list_t *the_node = (g_wiota_manager.current_freq_node == RT_NULL) ? (&g_wiota_manager.freq_list.node) : (&g_wiota_manager.current_freq_node->node);

    if (rt_list_isempty(&g_wiota_manager.freq_list.node))
    {
        rt_kprintf("choose_freq epty\n");
        return 1;
    }
...
static int at_wiota_manager_scan(void)
{
    ...
    default:
    {
        ...
        for (i = (id % subsystem_id_len); i < subsystem_id_len + (id % subsystem_id_len); i++)
        {
            ...
            rt_kprintf("scan frq res %d subid 0x%x t %d\n",
...
static int at_wiota_manager_run(void)
{
    ...
    at_wiota_set_state(AT_WIOTA_RUN);
    uc_wiota_connect();
    return uc_wiota_wait_sync(uc_wiota_get_frame_len() / 100, 2);
}
...
static void at_wiota_manager_startegy(void)
{
    ...
    while (1)
    {
        ...
        else
        {
            ...
            uc_wiota_status_e connect_state;

            // get state
            connect_state = uc_wiota_get_state();
            ...
            {
                rt_kprintf("startegy Su %d T%d S %d\n",
